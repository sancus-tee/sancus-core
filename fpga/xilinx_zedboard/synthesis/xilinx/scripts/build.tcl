...
proc print_help {} {
  ...
  puts "$script_file -tclargs \[--pmem_size <size>\]"
  puts "$script_file -tclargs \[--dmem_size <size>\]"
  puts "$script_file -tclargs \[--help\]\n"
  puts "Usage:"
  puts "Name                    Description"
  puts "-------------------------------------------------------------------------"
  puts "\[--origin_dir <path>\]   Determine source file paths with respect this"
  puts "                        path. Default path is the directory where this"
  puts "                        script is located.\n"
  puts "\[--project_name <name>\] Create project with the specified name. Default"
  puts "                        name is the name of the project from where this"
  puts "                        script was generated.\n"
  puts "\[--pmem_size <size\]     Configure the required program memory size (in"
  puts "                        bytes). Make sure to uncomment the relevant line"
  puts "                        in \"openMSP430_defines.v\". Default size is"
  puts "                        41924 bytes, or 41 KB.\n"
  puts "\[--dmem_size <size\]     Configure the required data memory size (in"
  puts "                        bytes). Make sure to uncomment the relevant line"
  puts "                        in \"openMSP430_defines.v\". Default size is"
  puts "                        16318 bytes, or 16 KB.\n"
...
if { $::argc > 0 } {
  for {set i 0} {$i < $::argc} {incr i} {
    ...
    switch -regexp -- $option {
      ...
      "--pmem_size"    { incr i; set pmem_size [lindex $::argv $i] }
      "--dmem_size"    { incr i; set dmem_size [lindex $::argv $i] }
      "--help"         { print_help }
      default {
        if { [regexp {^-} $option] } {
...
set files [list \
 ...
 [file normalize "${origin_dir}/../../../rtl/verilog/io_mux.v"] \
 ...
 [file normalize "${origin_dir}/../../../rtl/verilog/omsp_uart.v"] \
 ...
 [file normalize "${origin_dir}/../../../rtl/verilog/openMSP430_fpga.v"] \
...
set file "[file normalize "${origin_dir}/openMSP430_fpga.xdc"]"
set file_added [add_files -norecurse -fileset $obj [list $file]]
set file "${origin_dir}/openMSP430_fpga.xdc"
...
# Create block design
source [file normalize "${origin_dir}/design_1.tcl"]

# Set 'pmem_gen_0' properties
set obj [get_bd_cell pmem_gen_0]
set_property -name "CONFIG.Write_Depth_A" -value [expr $pmem_size/2] -objects $obj

# Set 'dmem_gen_0' properties
set obj [get_bd_cell dmem_gen_0]
set_property -name "CONFIG.Write_Depth_A" -value [expr $dmem_size/2] -objects $obj
